...
triage 実施: 2026-07-17 11:18（後続 cycle approval-reorder の orchestrate Block 0 codify gate で処理）。recurrence pre-triage + autonomous triage、質問 0 件。実装は次の codify 実装 cycle。

### Insight 1（多段 pipe の rc ガードは先頭コマンド単独実行で rc 先取り、エラー経路 oracle は権限拒否 fixture で誘発）
- **Decision**: codified
- **Destination**: rule (rules/test-patterns.md + .claude/rules/ mirror)
- **Reason**: pipe-rc masking family の再発（`bash subject | grep -q` 禁止 / SIGPIPE / command-substitution fallback に続く 4 例目）→ pre-triage 2+ 基準で自動昇格。追記内容: (a) rc を検査するコマンドは pipe に入れず「単独実行 + rc 先取り → フィルタ分離」、(b) rc>=2 経路の oracle は不在パスでは誘発できない場合がある（grep -r --include は不在 dir で silent rc=1 の実測）— 権限拒否 fixture を使う
- **Decided**: 2026-07-17 11:18

### Insight 2（連番・カウントの「次の値」は当該ファイルの実装 grep で実測。ヘッダ一覧は drift 前提で扱う）
- **Decision**: codified
- **Destination**: rule (rules/plan-discipline.md + .claude/rules/ mirror)
- **Reason**: novel だが「narrative な baseline 記述禁止」の識別子版として即 harden 可能。TC-16 番号衝突で Codex plan review BLOCK の実害 evidence あり（doc drift 修正 cycle 自身が stale ヘッダに騙された再帰事例）
- **Decided**: 2026-07-17 11:18

### Insight 3（negative sweep パターンは置換後新文言への不一致を oracle 確認、reviewer 対案も無検証で採用しない）
- **Decision**: codified
- **Destination**: rule (rules/test-patterns.md + .claude/rules/ mirror)
- **Reason**: oracle 実測 family の再発（printf oracle / literal pre-existing count 実測に続く 3 例目）→ 自動昇格。Codex 提案パターン自体に false-positive 欠陥があった実例（指摘が正しくても対案が正しいとは限らない）
- **Decided**: 2026-07-17 11:18

### 成功事例（observation: competitive review が全層で機能）
- **Decision**: no-codify
- **Reason**: 既存 pipeline の追認（Retrospective 内で no-codify 明記済み）
- **Decided**: 2026-07-17 11:18
