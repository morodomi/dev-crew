...
- **reject 除外の意味論変更**: 3-category triage で `reject` に分類された finding は、severity に関わらず verdict の集計から完全に除外される。**例**: severity=critical の finding が `reject` に分類された場合、他に critical/important の accepted finding が無ければ verdict は **PASS**（旧 blocking_score 方式では reviewer の自己申告スコアがそのまま反映され得たが、新方式は accept-apply/accept-defer のみを見るため reject された critical は verdict に一切効かない）
