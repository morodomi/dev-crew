...
# TC-19: docs/STATUS.md contains "Skills | 28" AND "Test Scripts | 116", README.md contains "28 skills"
...
# Updated 2026-09-03: 115→116 (test-severity-verdict.sh added)
echo ""
echo "TC-19: STATUS.md Skills=28 + Test Scripts=116, README.md 28 skills"
if [ ! -f "$STATUS_MD" ]; then
  fail "TC-19: docs/STATUS.md does not exist"
else
  has_skills28=$(grep -qE "Skills[[:space:]]*\|[[:space:]]*28" "$STATUS_MD" && echo "yes" || echo "no")
  has_scripts115=$(grep -qE "Test Scripts[[:space:]]*\|[[:space:]]*116" "$STATUS_MD" && echo "yes" || echo "no")
  ...
  if [ "$has_skills28" = "yes" ] && [ "$has_scripts115" = "yes" ] && [ "$has_readme28" = "yes" ]; then
    pass "TC-19: STATUS.md Skills=28 + Test Scripts=116, README.md 28 skills"
  else
    ...
    fail "TC-19: STATUS.md Skills=$skills_current (need 28), Test Scripts=$scripts_current (need 116), README=$readme_current (need '28 skills')"
  fi
fi
