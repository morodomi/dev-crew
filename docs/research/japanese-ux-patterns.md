...
**Japanese Pattern**: 文化非依存の AI生成 UI レビュー観点 (P-01〜P-12 の Japanese Pattern 列とは性質が異なる)。日本語向け UI でも英語向け UI でも同じく適用。

**Western Pattern**: 同上 (文化非依存)。

**Implementation Guidelines**:
- hero element を 1 つに絞る (font-size を主役のみ大きく、他要素を sub に)
- visual weight: secondary 要素の opacity を 0.7-0.8 に下げる
- CTA: primary 1 つ + secondary 1 つの 2 階層に制限

**Examples**:
- Stripe ダッシュボード: 売上数値が hero、補助 chart は muted style
- Notion 新規ページ: タイトル入力 hero + AI 提案 secondary
...
**Japanese Pattern**: 文化非依存。

**Western Pattern**: 文化非依存。

**Implementation Guidelines**:
- ドメイン用語をそのまま label に使う (一般用語に翻訳しない)
- 業務フロー順に UI 要素を配置 (alphabetical / generic order を避ける)
- domain-specific shortcut (例: 経理向けの伝票番号入力フィールド) を含める

**Examples**:
- 医療カルテ: 薬剤相互作用警告 + ICD-10 コード入力
- 経理仕訳画面: 伝票番号スキャナ連携 + 勘定科目テンプレート
...
**Japanese Pattern**: 文化非依存 (P-03 Color Palette とは別観点で色役割分離)。

**Western Pattern**: 文化非依存。

**Implementation Guidelines**:
- 1 色 1 役割: brand / action / state / warning を別 token で定義
- semantic color naming: --color-action-primary、--color-state-success 等
- 同色の重複使用禁止 (例: brand blue を link にも使わない)

**Examples**:
- Material Design: surface / primary / error / warning が独立 token
- Tailwind: blue-500 (action) + indigo-500 (brand) を区別
...
**Japanese Pattern**: 文化非依存 (CJK 長文の崩れ検証は P-02 typography で扱う)。

**Western Pattern**: 文化非依存。

**Implementation Guidelines**:
- text-overflow: ellipsis + max-width で長文対応
- empty-state component: data 0 件時の専用 UI
- skeleton loader: missing values の placeholder
- numeric formatter: K/M/B suffix で異常値対応

**Examples**:
- Notion: long title は 2 行 ellipsis、tag は overflow scroll
- Linear: 0 issue 時に illustrated empty state
...
**Japanese Pattern**: 文化非依存 (P-01 Information Density とは別観点で装飾削減)。

**Western Pattern**: 文化非依存。

**Implementation Guidelines**:
- gradient/shadow/border-radius は purpose-driven のみ
- chrome 要素 (header/sidebar/footer) を content より背景化
- onboarding tooltip は 1 task = 1 tooltip max

**Examples**:
- Linear: 余計な border/shadow なし、content first
- Figma: chrome 最小化、tool palette は context-dependent expand
