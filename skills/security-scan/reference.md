...
コンテキスト圧縮後に安全に再開するためのガイダンス。

### 圧縮境界

```
RECON → [compact possible] → SCAN/ATTACK (parallel) → [compact possible] → REPORT
```

### RECON 完了後

以下が確認できれば、圧縮後も SCAN フェーズから安全に再開できる:

- [ ] recon-agent の出力（endpoints, tech_stack, attack_priorities）が記録済み
- [ ] framework 情報が特定済み
- [ ] high_priority_count が集計済み

### SCAN/ATTACK 完了後

以下が確認できれば、圧縮後も REPORT フェーズから安全に再開できる:

- [ ] 全エージェントの検出結果が vulnerabilities 配列に記録済み
- [ ] summary（total, critical, high, medium, low）が集計済み
- [ ] verification 結果が記録済み（--dynamic 使用時）
