# Archived Roadmap: v2 / v2.4 / v2.5 / v2.6 / v2.6.x / v2.7 / v3
...
## v2.6.x (完了): 構造厳格化 + Verification + Automation

### v2.6.x Phase 30: ディレクトリ構造厳格化 (完了)

docs/cycles/ の構造規約をルール化し、検証スクリプトで決定論的に強制。test-directory-structure.sh 新設(9テスト)。validate-cycle-frontmatter.sh にfield存在チェック追加。旧形式Cycle doc 8件に phase: DONE frontmatter追加。onboard validation/reference に命名規約反映。

### v2.6.x Phase 31: Product Verification PoC (完了)

REFACTOR→REVIEW間にadvisory Product Verificationステップ(Block 2c.5)を追加。Cycle doc `## Verification` セクション駆動。セクション不在→サイレントスキップ。Evidence保存先: `/tmp/dev-crew-verify-{cycle-id}/`。orchestrate SKILL.md/reference.md/steps-*.md に VERIFY ステップ追加。test-product-verify.sh 新設(9テスト)。

### v2.6.x #84: designer AI生成UIレビュー観点 (完了)

designer agentに新カテゴリ「AI-Generated UI Review」(P-13〜P-17)を追加。Priority Focus, Context-Driven Design, Color Role Separation, Real Data Resilience, Ruthless Elimination。japanese-ux-patterns.md に詳細ガイドライン追加。

### v2.6.x #102: pre-red/pre-commit gate修正 (完了)

旧形式Cycle doc(phase:フィールドなし)をactiveとして誤検出するバグを修正。phase:フィールドを明示的に抽出し、不在docをスキップ。
