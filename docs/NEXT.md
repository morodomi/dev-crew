...
| 優先 | 内容 |
|---|---|
| 1 | **`skills/spec` と `skills/review` に複雑さ予算と Change Card を組み込む**（下記）。以降の cycle をこの規律下で回す |
| 2 | **`run-tests.sh` の縮小**（目標 250〜400 行）。あわせて false-PASS 経路 2 件を修正する。退避済み: `git stash list` の hotfix |
| 3 | 境界決定の ADR を書く: 「テスト実行の隔離は snapshot までとし、排他ロック・所有者プロトコルは必要性が実測されるまで採らない」 |

**縮小の内訳**（Codex 案。私も論理を確認済み）
...
| **manifest C** | **削る** | コピー中に変われば snapshot は混成になり `B≠A` で捕まる。変わって戻った場合（ABA）は snapshot の内容が A と一致し、そもそも一貫している。**C は「live tree が静かだった」ことしか証明せず、安全な実行まで拒否する** |
| **PID・起動時刻・owner・ABA 対策 reaper** | **削る** | kill -9 後の一時コピーは失ってよいデータ。起動時に専用 prefix の古い dir を消せば足りる |
...
「kill 耐性」の定義も下げる。**「絶対に残骸を残さない」ではなく「残骸が次回までに回収され、害を及ぼさない」**。
...
- **review**: 指摘ごとに必ず 3 択を出す — **(1) 今回直す (2) 削る・単純化して直す (3) リスクを明記して受容する**。あわせて「放置時の発現条件・影響・実測根拠」を 1 行
- 行数は拒否条件ではなく**警報**。超えたら自動 BLOCK ではなく Change Card と規模判断を必須にする
- 予算超過時の人間への確認は **cycle ごとに 1 回だけ**（そうしないと今回の 9 回が再来する）
