...
保存先: `.claude/meta-skills/instincts/`（プロジェクトローカル）

選択理由:
- プロジェクト内に閉じるため、プロジェクト固有のパターンが他プロジェクトと混在しない
- Git 管理が可能（`.gitignore` で公開制御できる）
- learn/evolve 両スキルが同一パスを参照し、依存関係がシンプル
