...
- プロジェクト目的の確認（AskUserQuestion: 「このプロジェクトは何を実現するためのものですか？（1-2文で）」）
- フレームワーク・パッケージマネージャの確認

#### existing-no-tdd モード

- AGENTS.md Overview にプロジェクト目的記述がなければ、freshモードと同様に質問
...
- Overview の `${PROJECT_PURPOSE}` 有無チェック（不在なら追加提案）
...
### CLAUDE.md マージ戦略 (最大3セクション)

| セクション | 戦略 |
|-----------|------|
| `@AGENTS.md` import | 先頭行に配置（必須） |
| AI Behavior Principles | 新規で上書き |
| Codex Integration | 新規で上書き（Codex 利用可能時） |
| カスタムセクション | 既存を保持 |
...
| 6 | TDD Workflow sync-plan 更新チェック | AGENTS.md TDD Workflow | TDD Workflow に sync-plan ステップが含まれているか確認し、不在なら追加提案 |
...
`@AGENTS.md` import, AI Behavior Principles, Codex Integration (最大3セクション)。
...
- **Codex Integration セクション**: Codex が利用可能なプロジェクトでは、以下のリテラルテンプレートをそのまま使用する（auto-orchestrate トリガー行を含む）:
...
2. `dev-crew-installed` モード: 既存 CLAUDE.md の TDD 関連セクションを AGENTS.md に分離。CLAUDE.md は `@AGENTS.md` import + AI Behavior Principles + Codex Integration（利用可能時）に簡素化。
...
| `${PROJECT_PURPOSE}` | 競馬予想の自動配信システム | `[要設定]` |
...
## sync-skills 誘導 {#sync-skills-prompt}

Step 9 完了メッセージ表示後に実行するオプショナルなCodex連携誘導。

### 条件

| # | チェック | 結果 | アクション |
|---|---------|------|-----------|
| 1 | `command -v codex` | 不在 | スキップ |
| 2 | `.agents/skills/` 存在 | 存在する | スキップ（同期済み） |
| 3 | 上記両方クリア | - | AskUserQuestion で確認 |

### プロンプト

AskUserQuestion:
「Codexがインストールされていますが、.agents/skills/ が未作成です。
Codex用にスキルを同期しますか？（sync-skillsを実行します）」

### 実行

ユーザーOK → `Skill(dev-crew:sync-skills)` を実行。
ユーザーNo → スキップして完了。
