...
**最初に実行**: TaskCreate で TDD サイクルのタスクを登録する。詳細: [reference.md](reference.md#task-list)
...
Cycle doc `## Verification` セクションのコマンドを実行。advisory evidence（非ブロッキング）。詳細: [reference.md](reference.md#product-verification)。セクション不在 or コマンドなし → サイレントスキップ。
...
Skill(dev-crew:cycle-retrospective): 失敗-成功 insight を抽出、retro_status を captured/resolved に遷移。abort signal (exit 1) を受信した場合は、既にユーザーが skill 内 AskUserQuestion で abort を選択した後なので、orchestrate は再確認せず COMMIT を停止して「/orchestrate を再起動してください」と出力して終了。詳細: [reference.md](reference.md#block-2f)
