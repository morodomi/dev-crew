...
bash <repo>/tests/test-frontmatter-retro-status.sh
...
for f in <repo>/docs/cycles/*.md; do
  bash <repo>/scripts/validate-cycle-frontmatter.sh "$f" || echo "FAIL: $f"
done

# 4. 構造検査
grep -n "retro_status" <repo>/skills/spec/templates/cycle.md
grep -n "retro_status" <repo>/agents/sync-plan.md
grep -n "retro_status" <repo>/rules/state-ownership.md
```
