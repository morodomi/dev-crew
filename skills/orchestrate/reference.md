...
|-------|-------|--------|
| INIT | PdM (Lead) 直接実行 | - |
| PLAN | PdM → architect | Skill(plan) |
| review(plan) | PdM → risk-based reviewer | 討論/並列 |
| RED | PdM → N red-worker | 並列テスト作成 |
| GREEN | PdM → N green-worker | 並列実装 |
| REFACTOR | PdM → refactorer | Skill(refactor) |
...
| 場面 | 再試行上限 | 超過時のアクション |
|------|-----------|-------------------|
| review(plan) BLOCK | max 1回再試行 | ユーザーに報告、PLAN 修正を依頼 |
...
- [x] Block 1: INIT → PLAN → review(plan) → PASS
...
Socrates がタイムアウトまたは異常応答を返した場合、v5.0 互換ロジックにフォールバックする:

| 障害 | フォールバック動作 |
|------|-------------------|
| Socrates 無応答/タイムアウト | WARN → 自動進行、BLOCK → 自動再試行 |
...
Protocol フローの Step 2 で応答がない場合、Step 3-4 をスキップしてフォールバックする。
