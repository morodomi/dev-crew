...
**Auto-orchestrate after plan approve**: plan approve後は `/orchestrate` を起動する。orchestrate が sync-plan → plan-review → TDDサイクルを全て管理する。Edit/Write は orchestrate 起動まで hook (post-approve-gate.sh) でブロックされる。
