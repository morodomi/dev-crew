...
```
Teammate A (Backend):
  → Skill(core:red) with backend test cases
  → Skill(core:green) with backend implementation
  → Skill(core:refactor)

Teammate B (Frontend):
  → Skill(core:red) with frontend test cases
  → Skill(core:green) with frontend implementation
  → Skill(core:refactor)
```
...
1. Skill(core:red) を実行（全レイヤーのテスト作成）
2. Skill(core:green) を実行（全レイヤーの実装）
3. Skill(core:refactor) を実行

次のアクション: Skill(core:red) を個別実行してください。
```
