...
走る前に、他のテストプロセスが既に走っていないかだけを見て、走っていれば止まる（多重起動の抑止）。実行はリポジトリのコピーの上で行うので、走っている最中にファイルを触っても結果は汚れない。強制終了などでコピーの残骸が残っていたら、削除はせず起動時に警告する。
...
|---|---|
| 0 | 全部通った |
| 1 | 実行したテストのうち 1 件以上が落ちた |
| 2 | runner が開始できなかった（多重起動・コピー失敗など） |
| 3 | 引数がおかしい |

シグナルで止まった場合は別枠（TERM=143 / INT=130 / HUP=129）。
...
| 2026-09-16 | `run-tests.sh` / テストの入れ子 | 前回追加した指紋の三点照合・再試行・PID/所有者追跡・reaper・load/memory 判定を削除し、入れ子実行（`test-doc-consistency.sh` TC-13 / `test-factory-model-adaptation.sh` TC-14 が全スイートを再実行していた構造）も除去。終了コードを 0/1/2/3 に縮小。[cycle doc](cycles/20260916_1634_shrink-runner-remove-nesting.md) |

前回（2026-09-13）は「遅さ」を測らずに整合機構を作ってしまい、`run-tests.sh` が肥大化した。今回はまず実測し（入れ子込みで延べ 458 回実行していたテストが、除去後は 117 回になる）、実測に基づいて削れるものだけを削った。
