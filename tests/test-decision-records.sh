...
# TC-05: kickoff/reference.md references TEMPLATE.md
echo ""
echo "TC-05: kickoff/reference.md references TEMPLATE.md"
if grep -q "docs/decisions/TEMPLATE.md" "$KICKOFF_REF"; then
  pass "TC-05: kickoff/reference.md references TEMPLATE.md"
else
  fail "TC-05: kickoff/reference.md does not reference TEMPLATE.md"
fi
...
# TC-09: kickoff/reference.md retains ADR creation conditions (3 triggers)
echo ""
echo "TC-09: kickoff/reference.md ADR creation conditions"
has_multi_cycle=false
has_override=false
has_deferred=false
if grep -q "複数サイクルに影響" "$KICKOFF_REF"; then has_multi_cycle=true; fi
if grep -q "過去のADRを覆す" "$KICKOFF_REF"; then has_override=true; fi
if grep -q "人間がDeferred判断" "$KICKOFF_REF"; then has_deferred=true; fi
if $has_multi_cycle && $has_override && $has_deferred; then
  pass "TC-09: kickoff/reference.md retains all 3 ADR creation conditions"
else
  fail "TC-09: kickoff/reference.md missing ADR creation conditions (multi=$has_multi_cycle override=$has_override deferred=$has_deferred)"
fi
...
# TC-11: Regression - existing kickoff debate tests still pass
echo ""
echo "TC-11: Regression test-kickoff-debate.sh"
if bash "$BASE_DIR/tests/test-kickoff-debate.sh" > /dev/null 2>&1; then
