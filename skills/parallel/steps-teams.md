...
```
Teammate(operation: "spawnTeam", team_name: "parallel-dev")
```
...
```
Teammate A (Backend):
  → Skill(dev-crew:red) with backend test cases
  → Skill(dev-crew:green) with backend implementation
  → Skill(dev-crew:refactor)

Teammate B (Frontend):
  → Skill(dev-crew:red) with frontend test cases
  → Skill(dev-crew:green) with frontend implementation
  → Skill(dev-crew:refactor)
```
...
1. Skill(dev-crew:red) を実行（全レイヤーのテスト作成）
2. Skill(dev-crew:green) を実行（全レイヤーの実装）
3. Skill(dev-crew:refactor) を実行

次のアクション: Skill(dev-crew:red) を個別実行してください。
```
